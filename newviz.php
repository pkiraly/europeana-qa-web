...
function getCountFromRGeneratedJson($filePrefix, &$errors) {
  ...
  if (file_exists($jsonCountFileName)) {
    ...
    $msg = sprintf("%s:%d file %s is not existing", basename(__FILE__), __LINE__, $jsonCountFileName);
    $errors[] = $msg;
    error_log($msg);
  }
  return $count;
...
function getEntityCountsFromRGeneratedJson($filePrefix, &$errors) {
  ...
  if (file_exists($jsonFreqFileName)) {
    ...
    $msg = sprintf("%s:%d file %s is not existing", basename(__FILE__), __LINE__, $jsonFreqFileName);
    $errors[] = $msg;
    error_log($msg);
  }
  return $entityCounts;
